...
class SimilarityDetector:
        ...
        try:
            ...
            for node in ast.walk(tree):
                if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef, ast.ClassDef)):
                    ...
                    if end_line - start_line < self.config["complexity_settings"]["min_lines"]:
                        continue
                    ...
                    code_block = {
                        ...
                        "signature": self._get_function_signature(node) if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)) else None
                    }
                    ...
                    if code_block["signature"]:
                        self.function_signatures[code_block["signature"]].append(code_block)
    ...
    def find_similar_blocks_parallel(self) -> List[Dict]:
        ...
        similarity_threshold = self.config["similarity_settings"]["similarity_threshold"]
        ...
        for hash_code, blocks in hash_groups.items():
            if len(blocks) > 1:
                ...
                group = {
                    ...
                    "blocks": blocks,
                    "optimization_potential": self._calculate_optimization_potential(blocks)
                }
        ...
        if remaining_blocks:
            try:
                print(f"🔄 建立 SimHash 索引用於 {len(remaining_blocks)} 個程式碼塊的快速查找...")
                
                # 建立 SimHash 索引
                simhash_objs = [(block["hash"], Simhash(block["normalized_code"])) for block in remaining_blocks]
                simhash_index = SimhashIndex(simhash_objs, k=simhash_threshold)
                
                # ... 現有的 SimHash 處理邏輯 ...
                
            except Exception as e:
                print(f"⚠️ SimHash 索引失敗: {e}，跳過相似度檢測，只返回完全重複的結果...")
                # 不進行序列相似度檢測，直接返回完全重複的結果
                pass
        
        print(f"🚀 SimHash 優化完成！從 O(N²) 降級為接近 O(N) 的查找效率")
        return similar_groups
    
    def _find_similar_blocks_sequential(self, blocks: List[Dict], processed_hashes: set) -> List[Dict]:
        """序列方式找出相似的程式碼塊（優化版本）"""
        similar_groups = []
        similarity_threshold = self.config["similarity_settings"]["similarity_threshold"]
        ...
        # 預過濾：只處理足夠長的程式碼塊
        filtered_blocks = [block for block in blocks if len(block["normalized_code"]) >= min_code_length]
            ...
            for i, block1 in enumerate(filtered_blocks):
                ...
                if similar_blocks:
                    group = {
                        ...
                        "optimization_potential": self._calculate_optimization_potential([block1] + similar_blocks)
                    }
    ...
    def _calculate_optimization_potential(self, blocks: List[Dict]) -> Dict:
        """計算優化潛力"""
        if not blocks:
            return {"duplicate_lines": 0, "potential_savings": "0 行程式碼", "refactor_suggestion": "無"}
        
        # 計算重複的行數
        first_block = blocks[0]
        total_lines = sum(block["end_line"] - block["start_line"] for block in blocks)
        duplicate_lines = total_lines - (first_block["end_line"] - first_block["start_line"])
    ...
    def run_analysis(self) -> Dict:
        ...
        total_savings = sum(group["optimization_potential"]["duplicate_lines"] for group in similar_groups)
        ...
        results = {
            "summary": {
                ...
                "files_analyzed": len(set(block["file_path"] for block in self.code_blocks))
            },
