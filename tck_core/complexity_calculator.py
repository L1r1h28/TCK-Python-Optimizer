...
class ComplexityCalculator:
    ...
    def run_analysis(self) -> Dict:
        ...
        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            # 提交所有任務
            future_to_file = {
                executor.submit(analyze_file_worker, task): task[0] for task in tasks
            }

            # 使用 tqdm 追蹤進度
            with tqdm(
                total=len(python_files), desc="🔧 複雜度分析", unit="檔案"
            ) as pbar:
                # 處理完成的任務
...
def main():
    ...
    if results["high_complexity_functions"]:
        ...
        for func in results["high_complexity_functions"][:5]:
            print(
                f"  - {func['name']} ({func['time_complexity']}) - {os.path.basename(func['file_path'])}"
            )
